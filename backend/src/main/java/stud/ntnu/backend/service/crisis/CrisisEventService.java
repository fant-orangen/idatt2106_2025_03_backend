...
/**
 * Service responsible for managing crisis events in the system. Provides functionality for
 * creating, updating, deactivating, and querying crisis events, as well as managing notifications
...
public class CrisisEventService {
  ...
  /**
   * Deactivates a crisis event and notifies affected users. The event is not deleted but marked as
   * inactive, preserving historical data.
   *
  ...
  /**
   * Updates an existing crisis event and notifies affected users of changes. The start time of a
   * crisis event cannot be updated after creation.
   *
  ...
  /**
   * Retrieves crisis events affecting a specific user. A crisis event affects a user if their
   * household location is within the event's radius.
   *
   ...
   */
  @Transactional(readOnly = true)
  public Page<CrisisEvent> searchCrisisEvents(String searchTerm, boolean isActive,
      Pageable pageable) {
