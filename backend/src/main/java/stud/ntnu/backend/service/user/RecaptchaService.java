...
/**
 * Service responsible for verifying Google reCAPTCHA tokens. This service handles the communication
 * with Google's reCAPTCHA API to validate tokens and determine if a user's interaction is
 * legitimate based on a score threshold.
 */
@Service
public class RecaptchaService {
  ...
  /**
   * Verifies a reCAPTCHA token by sending it to Google's verification API. The verification process
   * checks both the token's validity and the associated risk score. A score of 0.5 or higher is
   * considered acceptable for most use cases.
   *
   * @param recaptchaToken The reCAPTCHA token to verify, obtained from the client-side reCAPTCHA
   *                       widget
