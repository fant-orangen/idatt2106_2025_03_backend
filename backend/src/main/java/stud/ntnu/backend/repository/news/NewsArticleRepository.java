...
public interface NewsArticleRepository extends JpaRepository<NewsArticle, Long> {
   ...
   * @return a page of news articles
   */
  Page<NewsArticle> findByCrisisEventIdAndStatus(Integer crisisEventId, ArticleStatus status,
      Pageable pageable);
  ...
  /**
   * Find all news articles for a specific crisis event ordered by published date (newest first)
   * with pagination.
   *
   * @param crisisEventId the crisis event ID
   * @param pageable      pagination information
  ...
  Page<NewsArticle> findByCrisisEventIdOrderByPublishedAtDesc(
      @Param("crisisEventId") Integer crisisEventId, Pageable pageable);
  ...
  Page<NewsArticle> findByStatusOrderByCreatedAtDesc(@Param("status") ArticleStatus status,
      Pageable pageable);

  /**
   * Find all news articles with a specific status ordered by published date (newest first) with
   * pagination.
   *
   * @param status   the article status
   * @param pageable pagination information
