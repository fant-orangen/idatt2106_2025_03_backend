...
public interface ReflectionRepository extends JpaRepository<Reflection, Integer> {
   ...
   */
  Page<Reflection> findByUserIdAndDeletedFalseOrderByCreatedAtDesc(Integer userId,
      Pageable pageable);
  ...
  Page<Reflection> findSharedByHouseholdId(@Param("householdId") Integer householdId,
      Pageable pageable);

  /**
   * Find all shared and non-deleted reflections for users in households that are members of a
   * specific group.
   *
   ...
   */
  @Query(
      "SELECT r FROM Reflection r JOIN r.user u JOIN u.household h JOIN GroupMembership gm ON gm.household.id = h.id "
          +
  ...
  /**
   * Find all shared and non-deleted reflections that are visible to a specific user (from their
   * household and groups).
   *
   * @param userId   the ID of the user
   * @param pageable pagination information
   * @return a page of shared reflections
