...
public class AuthController {
  ...
  /**
   * Validates the JWT token from the Authorization header. Returns 200 OK if the token is valid,
   ...
   *
   * @param registrationRequest the registration request containing email, password, firstName,
   *                            lastName, and optional home address and location coordinates
  ...
  /**
   * Handles the email verification request. Receives the token sent via the verification link in
   * the email.
   *
  ...
  /**
   * Initiates the password reset process for a user. Sends a password reset email with a token
   * link.
   *
